...
public class MenuPause : MonoBehaviour
{
    ...
    [SerializeField] private Slider sensibilidadSlider; // Slider de sensibilidad
    private const string SensibilidadKey = "MouseSensitivity"; // Clave para guardar la sensibilidad
    ...
    {
        cronometro = FindObjectOfType<Cronometro>();
        playerMovement = FindObjectOfType<PlayerMovement>(); // Buscar PlayerMovement
        ...
        // Cargar la sensibilidad guardada o establecerla por defecto
        float savedSensibilidad = PlayerPrefs.GetFloat(SensibilidadKey, 100f);
        if (playerMovement != null)
        {
            playerMovement.mouseSensitivity = savedSensibilidad;
        }
        if (sensibilidadSlider != null)
        {
            sensibilidadSlider.value = savedSensibilidad;
            sensibilidadSlider.onValueChanged.AddListener(ActualizarSensibilidad);
        }
    ...
    private void ActualizarSensibilidad(float nuevaSensibilidad)
    {
        if (playerMovement != null)
        {
            playerMovement.mouseSensitivity = nuevaSensibilidad;
            PlayerPrefs.SetFloat(SensibilidadKey, nuevaSensibilidad);
        }
    }
}
